  ...
  return (
    <Navbar collapseOnSelect expand="lg" bg="dark" variant="dark">
      ...
      <Navbar.Collapse id="responsive-navbar-nav">
        <Nav>
          <Nav.Link onClick={() => setActivePage("messages")}>
            Messages
          </Nav.Link>
          <Nav.Link onClick={() => setActivePage("images")}>Images</Nav.Link>
          {/*here: separator*/}
          <Nav.Link onClick={() => setActivePage("carousel")}>
            Slider photo
          </Nav.Link>
          <Nav.Link onClick={() => setActivePage("biographie")}>
            Biographie
          </Nav.Link>
          <Nav.Link onClick={() => setActivePage("studies")}>Études</Nav.Link>
          <Nav.Link onClick={() => setActivePage("concerts")}>
            Concerts
          </Nav.Link>
          <Nav.Link onClick={() => setActivePage("repertory")}>
            Répertoire
          </Nav.Link>
          <Nav.Link onClick={() => setActivePage("videos")}>Vidéos</Nav.Link>
          <Nav.Link onClick={() => setActivePage("links")}>Liens</Nav.Link>
          <Nav.Link onClick={() => setActivePage("newsletter")}>
            Newsletter
          </Nav.Link>
        ...
        <Nav className="ml-auto">
          <Nav.Link onClick={disconnect}>
            <Button variant="info">Se déconnecter</Button>
          </Nav.Link>
        </Nav>
      </Navbar.Collapse>
    </Navbar>
...
class FeedbackManager {
  ...
  treatError(err) {
    ...
    if (err.response) {
      ...
      if (
        err.response.data &&
        (err.response.data.data === "Login required" ||
          (err.response.data.data &&
            (err.response.data.data.name === "TokenExpiredError" ||
              err.response.data.data.name === "JsonWebTokenError")))
      ) {
        sessionStorage.clear();
        const warningMessage = `Attention à bien sauvegarder toute donnée non enregistrée avant de se reconnecter !
...
Se reconnecter ?`;
        ...
        jsx = (
          <Button variant="warning" onClick={handleClickReconnect}>
            Se reconnecter
          </Button>
        );
      }
...
function ImageList(props) {
  ...
  useEffect(() => {
    if (Object.values(filter).filter((f) => f).length === 0) {
      ...
      setFilteredImages([
        ...images.filter(
          (img) =>
            (filter.banner && img.banner) ||
            (filter.concerts && img.concerts) ||
            (filter.uploads && img.fieldname)
        ),
      ]);
    }
  }, [filter]);
  ...
  return (
    <Container>
      <Form.Check
        type="checkbox"
        label="Images pour le slider photo"
        onChange={(e) => {
      ...
      <Form.Check
        type="checkbox"
        label="Images pour les concerts"
        onChange={(e) => {
          const checked = e.target.checked;
          setFilter({
            ...filter,
      ...
      <Form.Check
        type="checkbox"
        label="Images uploadées via l'interface d'admin"
        onChange={(e) => {
          const checked = e.target.checked;
          setFilter({
            ...filter,
      ...
      {filteredImages.map((img) => (
        <Image
          ...
          fluid
          key={img._id}
          src={img.destination + img.filename}
          thumbnail
          onClick={(e) => showPhotoModal(img)}
        />
      ))}
      <ImageModal
        show={showModal}
      ...
      <Modal.Body>
        <Card>
          ...
          <Card.Body>
            {image.banner && (
              <Alert variant="info">
                Peut être utilisée dans le slider photo
              </Alert>
            )}
            {image.concerts && (
              <Alert variant="info">
                Peut être utilisée pour illustrer un concert
              </Alert>
            )}
            <div style={{ overflow: "scroll" }}>
              <Table striped bordered hover>
                <tbody>
                  {Object.entries(image).map((e) => (
                    <tr key={e[0]}>
                      <th>{e[0]}</th>
                      <td>{e[1]}</td>
                    </tr>
                  ...
                  <tr key="url">
                    <th>url</th>
                    <td>
                      <Form.Control ref={urlControl} value={url} readOnly />
                    </td>
                  </tr>
                </tbody>
              </Table>
            ...
            <div className="float-right">
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  feedback.treatSuccess(
                    "URL de l'image copiée dans le presse-papier"
                  );
                  setShow(false);
                }}
              >
...
function Images(props) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!selectedFile)
      return feedback.treatError("Vous devez d'abord sélectionner une image.");
...
function MessageInfo(props) {
  ...
  return (
    <div style={{ color: stringToColour(ip) }}>{`${name} <${email}>`}</div>
  );
}
      ...
      <Card>
        <Card.Body>
          ...
          <Card.Subtitle className="mb-2 text-muted">
            ...
            <div>{`${
              ipInfos && ipInfos.city
                ? `Depuis ${ipInfos.city}, ${ipInfos.regionName}, ${ipInfos.country}`
                : `Adresse ip : ${ip}`
            }`}</div>
          </Card.Subtitle>
          <Card.Text as="div">
...
function Messages(props) {
  ...
  const handleClickMessage = (message) => {
    ...
    axios
      .patch(`/admin/message/read/${message._id}`)
      .then(function (response) {
        if (response.data) {
  ...
  const unreadMessage = () => {
    ...
    const messageId = selectedMessage._id;
    axios
      .patch(`/admin/message/unread/${messageId}`)
      .then(function (response) {
        if (response.data) {
          console.log(response.data);
          getMessages();
  ...
  return (
    <Container>
      <ButtonGroup aria-label="Basic example">
        <Button variant="primary" onClick={getMessages.bind(null, true)}>
          Rafraîchir
        </Button>
        {selectedMessage && selectedMessage.read && (
          <Button variant="info" onClick={unreadMessage}>
            Non lu
          </Button>
        )}
        {selectedMessage && (
        ...
        <Col lg={4}>
          <ListGroup>
            {messages.map((message) => (
              <ListGroup.Item
                active={selectedMessage && selectedMessage._id === message._id}
                variant={message.read ? "secondary" : "primary"}
  ...
  const getConcerts = () => {
    axios
      ...
      .then((res) => {
        ...
        if (
          selectedConcert !== undefined &&
          !res.data.map((c) => c._id).includes(selectedConcert._id)
        ) {
          setSelectedConcert();
        }
      })
    ...
    <Container>
      ...
      <ListGroup>
        {concerts.map((concert) => (
          <ListGroup.Item
            active={selectedConcert && concert._id === selectedConcert._id}
            key={concert._id}
            onClick={() => clickConcert(concert)}
            action
          >
            {concert.name}
          </ListGroup.Item>
      ...
      {concerts.filter((c) => c._id === "new").length === 0 && (
        <Button
  ...
  const saveConcert = (concertO) => {
    ...
    if (concert.details) {
      if (concert.details.artists instanceof Array) {
        concert.details.artists = concert.details.artists.filter(
          (artist) => artist.name || artist.instrument
        );
      }
      if (concert.details.pieces instanceof Array) {
        concert.details.pieces = concert.details.pieces.filter(
          (piece) => piece.title || piece.composer
        );
      }
    }
    axios
      .post("/admin/concerts", concertBd)
      .then((res) => {
        console.log(res);
      ...
      >
        ...
        <Form.Group>
          ...
          {concert.img ? (
            ...
            <Button onClick={selectImage} variant="outline-secondary">
              Ajouter une image
            </Button>
          )}
        </Form.Group>
        <Form.Group>
          ...
          <Form.Control
            ...
            rows={3}
            type="text"
            value={concert.info !== undefined ? concert.info : ""}
            onChange={(e) =>
              setConcert({
                ...concert,
                info: e.target.value,
        ...
        <br />
        {concert.details &&
          concert.details.artists &&
          concert.details.artists.length > 0 && (
            <Card body>
              <Form.Label>Artistes :</Form.Label>
              {concert.details.artists.map((artist, artIndex) => (
                <Card key={"artist-" + artIndex} body>
                  <Form.Group>
          ...
          onClick={(e) => {
            ...
            setConcert({
              ...
              details: {
                ...concert.details,
                artists:
                  concert.details && concert.details.artists
                    ? [...concert.details.artists, newArtist]
                    : [newArtist],
              },
            });
          }}
        ...
        {concert.details &&
          concert.details.pieces &&
          concert.details.pieces.length > 0 && (
            <Card body className="mt-2">
              <Form.Label>Œuvres :</Form.Label>
              {concert.details.pieces.map((piece, pIndex) => (
                <Card key={"piece-" + pIndex} body>
                  <Form.Group>
                    <Form.Label>Titre de l'œuvre :</Form.Label>
                    <Form.Control
                      key={"piece-title-" + pIndex}
                      type="text"
                      value={piece.title}
                      onChange={(e) => {
          ...
          onClick={(e) => {
            ...
            setConcert({
              ...
              details: {
                ...concert.details,
                pieces:
                  concert.details && concert.details.pieces
                    ? [...concert.details.pieces, newPiece]
                    : [newPiece],
              },
            });
          }}
...
function ConcertEdit(props) {
    ...
    if (concert && concert._id) {
      axios
        ...
        .then((res) =>
          setOccurrences(
            res.data
              .filter((c) => c.concertId && c.concertId === concert._id)
              .sort(
                (a, b) =>
  ...
  return concert ? (
    <Tabs variant="pills">
      ...
      <Tab
        eventKey="edit-occs"
        title="Éditer les occurences du concert"
        disabled={!concert._id || concert._id === "new"}
      >
        <ConcertOccurences
          concert={concert}
          feedback={feedback}
  ...
  return (
    <Container>
      <Form.Label className="text-danger">
        ...
        <br />
        Le concert <strong>{concert.name}</strong> et ses{" "}
        <strong>{occurrences.length}</strong> occurences seront perdues.
      </Form.Label>
      <br />
      <Button variant="danger" onClick={deleteConcert}>
    ...
    <Container>
      <Form
        onSubmit={(e) => {
          e.preventDefault();
          occurrences
            .filter((o) => o.modified && !o.deleted)
            .forEach((occ) => saveOccurence(occ));
          occurrences
            .filter((o) => o.deleted)
            .forEach((occ) => deleteOccurence(occ));
        }}
      >
        {occurrences.map((occ, index) => (
          <div
            key={occ._id ? occ._id : "key_" + index}
            className={occ.deleted ? "bg-secondary" : undefined}
          >
            <Form.Label>Occurrence n°{index + 1}</Form.Label>
            {fields.map((f) => (
              <Form.Group key={f.fieldname}>
                {f.type === "checkbox" ? (
                  <Form.Check
                    type={f.type}
                    label={f.description}
                    checked={occ[f.fieldname] || false}
                    onChange={(e) => {
                  ...
                  <div>
                    ...
                    <Form.Control
                      ...
                      value={occ[f.fieldname] ? occ[f.fieldname] : ""}
                      onChange={(e) => {
                        const occs = [...occurrences];
                        occs[index] = {
                          ...occs[index],
...
function Carousel(props) {
  ...
  const submitForm = (e) => {
    e.preventDefault();
    Object.values(data)
      .filter((el) => el.modified && !el.deleted)
      .forEach((el) => {
        feedback.clear();
        delete el.modified;
        delete el.deleted;
    ...
    <Container>
      <Form onSubmit={submitForm}>
        ...
        {Object.values(data)
          .sort(sortCarousel)
          .map((carouselElement) => (
            <Form.Group key={carouselElement._id}>
              <Row>
                <Col>
                  <Form.Control
                    placeholder={
                      carouselElement.title !== undefined
                        ? "Titre"
                        : "Image principale"
                    }
                    value={carouselElement.title}
                    onChange={(e) => {
                      setData({
                        ...data,
                        [carouselElement._id]: {
                  ...
                  <Row>
                    <Col sm>
                      ...
                      !(
                        carouselElement._id &&
                        carouselElement._id.startsWith("new")
                      ) ? (
                        !carouselElement.deleted ? (
                          <Button
                            className="mt-2"
                            variant="danger"
                            onClick={(e) => {
                          ...
                          <Alert className="mt-2" variant="warning">
                            <strong>Attention !</strong> cet élément sera
                    ...
                    <Col>
                      {!carouselElement.active && (
                        <Form.Control
                          className="d-flex mt-2"
                          type="number"
                          value={
                            carouselElement.index !== undefined
                ...
                <Col>
                  <Form.Control
                    ...
                    rows={3}
                    placeholder={
                      carouselElement.description !== undefined
                        ? "Description"
                        : ""
                    }
                    value={carouselElement.description}
                    onChange={(e) => {
                      setData({
                        ...data,
                        [carouselElement._id]: {
                ...
                <Col>
                  <Image
                    src={carouselElement.url}
                    thumbnail
                    onClick={(e) => showImageModal(carouselElement)}
                  />
                </Col>
              </Row>
              <hr />
...
function Bio(props) {
  ...
  const setBioData = (biobject) => {
    ...
    axios
      .post("/admin/biographie", dbBioObject)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getBioData();
      ...
      <Form onSubmit={submitForm}>
        ...
        {Object.values(paragraphs).map((p) => (
          <Form.Group
            key={p._id}
            className={
              !p.paragraph && p._id !== "new" ? "bg-secondary" : undefined
            }
          >
            ...
            onClick={(e) =>
              setParagraphs({
                ...
                new: {
                  ...
                  paragraph: "",
                  index:
                    Math.max(...Object.values(paragraphs).map((p) => p.index)) +
                    1,
                },
              })
            }
...
function Links(props) {
  ...
  return (
    <Container>
      <Form onSubmit={submitForm}>
        {linkTypes.map((type) => (
          <Form.Group key={type[0]}>
            <Row>
              <Form.Label>{type[1]}</Form.Label>
            </Row>
            {Object.values(links)
              .filter((link) => link.type === type[0])
              .map((link) => (
                <Row
                  className={
                    !link.name && !link._id.startsWith("new")
                      ? "bg-secondary"
                      : undefined
                  }
                  key={link._id}
...
function Studies(props) {
  ...
  const setStudData = (studobject) => {
    ...
    axios
      .post("/admin/studies", dbStudObject)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getStudData();
  ...
  const deleteStudData = (studobject) => {
    feedback.clear();
    axios
      .delete("/admin/studies/" + studobject._id)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getStudData();
    ...
    <Container>
      <Form onSubmit={submitForm}>
        ...
        {Object.values(awards).map((p) => (
          <Form.Group
            className={!p.award && p._id !== "new" ? "bg-secondary" : undefined}
            key={p._id}
          >
            <Form.Label>{`Récompense n°${p.index}`}</Form.Label>
            <Form.Control
              type="text"
              placeholder={`Récompense n°${p.index}`}
              value={p.award}
              onChange={(e) =>
                setAwards({
                  ...awards,
            ...
            onClick={(e) =>
              setAwards({
                ...
                new: {
                  ...
                  award: "",
                  index:
                    Math.max(...Object.values(awards).map((p) => p.index)) + 1,
                },
              })
            }
          >
...
function Repertory(props) {
  ...
  const deleteRepertoryItem = (rep) => {
    feedback.clear();
    axios
      .delete("/admin/repertory/" + rep._id)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getRepertory();
    ...
    <Container>
      ...
      <Form
        onSubmit={(e) => {
          e.preventDefault();
          repertory
            .filter((rep) => rep.modified && !rep.deleted)
            .forEach((rep) => {
              delete rep.modified;
              const repCopy = { ...rep };
      ...
      >
        {[...titles].map((title, titi) => (
          <Card key={"title-" + titi} body>
            <Form.Control
              type="text"
              ...
              onChange={(e) => {
                ...
                repertory
                  .filter((rep) => rep.title === title)
                  .forEach((rep) => {
                    const index = repertory.findIndex(
                      (_rep) => _rep._id === rep._id
                    );
                    repCopy[index] = {
                      ...rep,
                      title: e.target.value,
            ...
            {[
              ...
              <Card key={`subtitle-${titi}_${tits}`} body>
                {repertory
                  .filter((rep) => rep.title === title)
                  .map((e) => e.subtitle)
                  .filter((subtitle) => subtitle !== undefined).length < 1 ? (
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => {
                      ...
                      repertory
                        .filter(
                          (rep) =>
                            rep.title === title && rep.subtitle === subtitle
                        )
                        .forEach((rep) => {
                          const index = repertory.findIndex(
                            (_rep) => _rep._id === rep._id
                ...
                <ListGroup>
                  {repertory
                    .filter(
                      (rep) => rep.title === title && rep.subtitle === subtitle
                    )
                    .map((rep) => (
                      <ListGroup.Item
                        variant={rep.deleted ? "danger" : undefined}
                        key={rep._id}
                      >
                        <Form.Control
                          type="text"
                          placeholder="Élément de répertoire"
                          disabled={rep.deleted}
                          value={rep.content}
                          onChange={(e) => {
                            const repCopy = [...repertory];
                            const index = repertory.findIndex(
...
function VideoFormGroup(props) {
  ...
  const getThumbnail = () => {
    ...
    if (parsed.hostname === "vimeo.com") {
      ...
      fetch(`https://vimeo.com/api/v2/video${id_vimeo}.json`)
        .then((res) => res.json())
        .then((res) =>
          setVideo({
            ...video,
            ...
            description:
              !video.description && res[0].description
                ? res[0].description
                : video.description,
      ...
      fetch(`https://www.youtube.com/oembed?url=${video.url}`)
        .then((res) => res.json())
        .then((res) =>
          res.thumbnail_url
            ? setVideo({
      ...
      fetch(`https://noembed.com/embed?url=${video.url}`)
        .then((res) => res.json())
        .then((res) =>
          res.thumbnail_url
            ? setVideo({
  ...
  return (
    <div className={video.deleted ? "bg-secondary" : undefined}>
      <Form.Label>{`Vidéo n°${video.index}`}</Form.Label>
      <br />
      <Button
        variant={video.deleted ? "warning" : "danger"}
        onClick={(e) =>
          setVideo({
            ...video,
      ...
      >
        {video.deleted ? "Annuler la suppression" : "Supprimer"}
      </Button>
      <Form.Group>
        ...
        {!video.img && (
          <div>
            <Button onClick={() => getThumbnail()} disabled={!video.url}>
      ...
      <Form.Group>
        <Row>
          ...
          {video.img && (
            <Col>
              <Image src={video.img} thumbnail fluid className="float-right" />
            </Col>
          )}
        </Row>
        <Form.Control
...
function Videos(props) {
  ...
  const saveVideo = (video) => {
    feedback.clear();
    axios
      .post("/admin/video", video)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getVideos();
        setUpdated(
          Object.keys(videos).reduce((acc, curr) => {
            return {
              ...acc,
              [curr]: 0,
            };
    ...
    <Container>
      ...
      >
        {videos.map((video, index) => (
          <VideoFormGroup
            key={video._id ? video._id : index}
            video={video}
            setVideo={setVideo.bind(null, index)}
          />
        ))}
        <Button
          className="mt-2"
...
function Newsletter(props) {
  ...
  const extractSubscribers = () => {
    axios
      .post("/admin/extract_newsletter", subscribers)
      .then((res) => feedback.treatSuccess(res.data))
      .catch((err) => feedback.treatError(err));
  };
    ...
    <Container>
      ...
      <Form
        onSubmit={(e) => {
          ...
          axios
            .post("/admin/newsletter", newsletter)
            .then((res) => {
      ...
      >
        <Form.Group>
          ...
          <InputGroup>
            <Form.Control
              type="text"
              placeholder="Liste d'emails à qui envoyer la newsletter"
              value={newsletter.to}
              onChange={(e) =>
                setNewsletter({
                  ...newsletter,
              ...
              <Button
                ...
                onClick={(e) =>
                  setNewsletter({
                    ...newsletter,
                    to: subscribers
                      .filter((sub) => !sub.skip && !sub.unsubscribed)
                      .map((n) => n.email)
                      .join("; "),
                  })
                }
        ...
        <Form.Group>
          ...
          <Form.Control
            ...
            rows={5}
            type="text"
            placeholder="Texte de la newsletter..."
          ...
          <Card>
            <Card.Body
              dangerouslySetInnerHTML={{
                __html: converter.makeHtml(newsletter.message),
              }}
            ></Card.Body>
          </Card>
        </Form.Group>
        <Button
          variant="primary"
          type="submit"
          disabled={!newsletter.to || !newsletter.message}
        >
          Envoyer
        </Button>
      </Form>
      <hr />
      <h3>
        Liste des personnes inscrites aux newsletters :{" "}
        <Button variant="outline-info" onClick={extractSubscribers}>
          Extraire dans le Google Sheet
        </Button>
      </h3>
      <div style={{ overflow: "scroll" }}>
        <Table striped bordered hover>
          ...
          <tbody>
            {subscribers.map((sub, index) => (
              <tr key={sub._id}>
                <td>
                  ...
                  >
                    <Dropdown.Item
                      onClick={(e) => {
                        axios
                          .delete("/admin/newsletter/" + sub._id)
                          .then((res) => {
                    ...
                    <Dropdown.Item
                      onClick={(e) => {
                        axios
                          .delete("/admin/newsletter/byip/" + sub.ip)
                          .then((res) => {
                            if (res.data && res.data.removed) {
                              feedback.treatSuccess(
                                res.data.removed + " emails supprimés"
                              );
                            } else {
                              feedback.treatError("Erreur de suppression");
                            }
                    ...
                    {!sub.unsubscribed && (
                      ...
                      >
                        {sub.skip ? "Sélectionner" : "Désélectionner"}
                      </Dropdown.Item>
                    )}
                  </DropdownButton>
                </td>
                ...
                <td>
                  {sub.ipInfos
                    ? `${sub.ipInfos.city}, ${sub.ipInfos.regionName}, ${sub.ipInfos.country}`
                    : "Introuvable"}
                </td>
              </tr>
            ))}
...
function PageFormGroup(props) {
  ...
  const transformPageData = (pageData) => {
    return pageData.map((pe) =>
      pe.text ? { markdown: pe.text.join("\n\n"), legacy_text: pe.text } : pe
    );
  };

  const [data, setData] = useState(
    page.pageData && page.pageData.data
      ? transformPageData(page.pageData.data)
      : []
  );
  ...
  return (
    <Card body className="mt-2">
      ...
      <Form.Group controlId="url">
        ...
        <Form.Control
          type="text"
          placeholder="Entrez l'URL de la page"
          value={page.url}
      ...
      <Form.Group controlId="headerImage">
        ...
        <Form.Control
          type="text"
          placeholder="Entrez l'URL de l'image d'en-tête"
          value={page.pageData ? page.pageData.headerImageUrl : ""}
          onChange={(e) =>
            setPage({
              ...page,
              pageData: { ...page.pageData, headerImageUrl: e.target.value },
            })
          }
        />
      </Form.Group>
          ...
          {page.bgColor ? (
            <InputGroup>
              <Form.Control
                type="color"
                value={page.bgColor}
                onChange={(e) => setPage({ ...page, bgColor: e.target.value })}
              />
              <InputGroup.Append>
                <Button
                  variant="outline-secondary"
                  onClick={() => setPage({ ...page, bgColor: undefined })}
                >
            ...
            <Button
              variant="outline-secondary"
              onClick={() => setPage({ ...page, bgColor: "#d3d3d3" })}
            >
        ...
        data.map((item, index) => (
          <div key={index}>
                ...
                {key === "video" ? (
                  <div>
                    <Form.Group controlId={"videoUrl" + index}>
                      ...
                      <Form.Control
                        ...
                        onChange={(e) =>
                          setDataItem(index, {
                            ...item,
                            [key]: { ...item[key], url: e.target.value },
                          })
                        }
                      />
                    </Form.Group>
                    <Form.Group controlId={"videoThumbUrl" + index}>
                      ...
                      <Form.Control
                        type="text"
                        placeholder="Entrez l'URL de la vignette"
                        value={item[key].thumbUrl}
                        onChange={(e) =>
                          setDataItem(index, {
                            ...item,
                            [key]: { ...item[key], thumbUrl: e.target.value },
                          })
                        }
                      />
                    </Form.Group>
                  ...
                  <div>
                    {item[key].map((paragraph, subindex) => (
                      <div key={subindex}>
                        <Form.Group controlId={"text" + index + "-" + subindex}>
                          ...
                          <Form.Control
                            ...
                            value={paragraph}
                            onChange={(e) =>
                              setParagraph(index, subindex, e.target.value)
                            }
                          />
                        </Form.Group>
                        <Button
                          variant="outline-danger"
                          onClick={(e) => removeParagraph(index, subindex)}
                        >
                  ...
                  <div>
                    <Form.Group controlId={"markdown" + index}>
                      ...
                      <Form.Control
                        ...
                        value={item[key]}
                        onChange={(e) =>
                          setDataItem(index, { ...item, [key]: e.target.value })
                        }
                      />
                    </Form.Group>
                  </div>
            ...
            <ButtonGroup>
              <Button
                variant="outline-danger"
                onClick={(e) => removeDataItem(index)}
              >
                Supprimer
              </Button>
              {index > 0 && (
                <Button
                  variant="outline-secondary"
                  onClick={(e) => moveDataItem(index, -1)}
                >
                  Monter
                </Button>
              )}
      ...
      <ButtonGroup>
        <Button variant="outline-info" onClick={(e) => addDataItem("markdown")}>
          Ajouter du texte
        </Button>
        <Button variant="outline-info" onClick={(e) => addDataItem("image")}>
          Ajouter une image
        </Button>
        <Button variant="outline-info" onClick={(e) => addDataItem("video")}>
          Ajouter une vidéo
        </Button>
        <Button
          style={{ display: "none" }}
          disabled
          variant="outline-info"
          onClick={(e) => addDataItem("text")}
        >
          Ajouter du texte (legacy)
        </Button>
      </ButtonGroup>
      <Form.Group controlId="deleted">
        <Form.Check
          ...
          onChange={(e) => setPage({ ...page, deleted: e.target.checked })}
        />
      </Form.Group>
    </Card>
  );
...
function Pages(props) {
  ...
  const savePage = (page) => {
    feedback.clear();
    axios
      .post("/admin/page", page)
      .then((res) => {
        console.log(res);
        feedback.treatSuccess("Modifications effectuées !");
        getPages();
        setUpdated(
          Object.keys(pages).reduce((acc, curr) => {
            return {
              ...acc,
              [curr]: 0,
            };
    ...
    <Container>
      ...
      >
        {pages.map((page, index) => (
          <PageFormGroup
            key={page._id ? page._id : index}
            page={page}
            setPage={setPage.bind(null, index)}
          />
        ))}
        <Button
          className="mt-2"
...
function Content(props) {
  const startPage = () =>
    window.location.hash ? window.location.hash.replace("#", "") : "messages";
    ...
    <Container fluid>
      ...
      <Container>
        ...
        >
          ...
          <Toast.Body>
            <p>
              Le token de session ne sera plus valide dans{" "}
              <strong>{Math.floor(toastData.expMin)}</strong> minutes.
            </p>
            <p>
              Pensez à bien sauvegarder votre travail, ou vous déconnecter et
              vous reconnecter avant de continuer.
            </p>
          </Toast.Body>
        </Toast>
      </Container>
